...
try {
  ...
  await expect(page.locator('.layout-content')).toBeVisible();
	...
	console.log('SUCCESSFUL_TEST_EXECUTION:11');

} catch (error) {
	console.log('FAILED_TEST_EXECUTION:11');
...
try {
  ...
  // await expect(page.getByRole('button', { name: 'AutomatedTesting (Civils)  ' })).toBeVisible();
	...
	console.log('SUCCESSFUL_TEST_EXECUTION:12');

} catch (error) {
	console.log('FAILED_TEST_EXECUTION:12');
	console.error(error.error?.message || error.message);
}
