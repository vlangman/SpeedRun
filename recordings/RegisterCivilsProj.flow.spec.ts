...
test.describe('Register Civils Proj', async () => {
	...
	test('Create Civils Project LOCAL', async () => {
		try {
			...
			await page
				.getByRole('row', { name: /Expand Eastern Cape \(\d+\)/ })
				.getByLabel('Expand')
				.click();
			...
			await expect(
				page.getByText('Buffalo City, Eastern CapeBuffalo City, Eastern CapeBuffalo City, Buffalo City')
			).toBeVisible();
			...
			const contractsManagerDropdown = page.locator(
				'[id="projectForm:project-detail-tab-view:contract-manager"]'
			);
			await contractsManagerDropdown.click();
			const contractsManagerListbox = page.locator(
				'[id="projectForm:project-detail-tab-view:contract-manager_items"]'
			);
			...
			const financeManagerListbox = page.locator(
				'[id="projectForm:project-detail-tab-view:finance-manager_items"]'
			);
	...
	test('Delete Civils Project LOCAL', async () => {
		try {
